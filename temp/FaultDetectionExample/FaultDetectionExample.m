...
%% Introduction
% Early detection and isolation of anomalies in a machine's operation can
% help to reduce accidents, reduce downtime and thus save operational costs.
% The approach involves processing live measurements from a system's
...
% # Detection of deterioration by building models of a damaged system
...
% |idMechanicalSystem.slx| is a simple example of such a structure. The
...
%%
% Estimate a 7th order time-series model in state-space form using the
...
% future with > 85% accuracy. To view the fit to other experiments in the
...
% build 7th order time series models in state-space form. Turn off
% covariance computation for speed.
mNormal =  cell(nr,1);
mFault = cell(nr, 1);
nx = order(model);
...
% updates will have high variance while a large value will make it
...
%   adapting model of the system behavior. Preset thresholds on a model's
