...
P = diag([6^2 6^2 deg2rad(5) 1e-02 1e-02 deg2rad(2)...
    1e-04 1e-04 deg2rad(1) 1e-01 1e-01]);

% Process noise covariance.
Q = diag([1e-04 1e-04 deg2rad(2) 1e-06 1e-06 deg2rad(2)...
    1e-09 1e-09 deg2rad(2) 1e-05 1e-05]);

% Measurement noise covariance.
R = diag([6^2 6^2 deg2rad(5) deg2rad(2) 1e-01 1e-01]);
...
for ct = 1:numel(t)
   ...
   if t(ct) == 40   % Fault injection time, GPS is fixed after 35s
      %x1(3) = 3;  % Step change (Amount of fault)
   end
...
%% Detection Algorithm
   ...
   if t(ct) < 10 % Ts = 0.01 seconds
       ...
       % friction changes.
       estEst(ct) = mean(xSigEst(3,max(1,ct-10):ct));
...
figure; % Heading state estimation error
plot(t,xSigTrue(3,:)-xSigEst(3,:),'b','LineWidth',LineW); 
grid on; ax = gca; ax.FontSize = FontS_axis;
xlabel('time (s)','Fontsize',FontS_label); ylabel('Heading (rad)','Fontsize',FontS_label);

figure; % Residual = North measurement error
plot(t,ySig(3,:)-ySigEst(3,:),'b'); 
grid on; ax = gca; ax.FontSize = FontS_axis;
xlabel('time (s)','Fontsize',FontS_label); ylabel('Heading (rad)','Fontsize',FontS_label);

figure; % Attack Detection
plot(t,xSigEst(3,:),'b',...
