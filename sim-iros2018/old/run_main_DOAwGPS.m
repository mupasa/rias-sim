...
% setenv('ROS_IP','128.46.80.247') % MATLAB IP
% rosinit
...
% Attack Profile
att_t = 40;            % Injection time (sec)
att_mag = 3;           % Amount (meter)
att_robot = 1;         % Target robot
att_state = 2;         % Target state

fprintf('Attack Profile: %d sec, %d meter, Robot No. %d, State No. %d\n',...
...
P = diag([2.5^2 2.5^2 deg2rad(0.2) 1e-02 1e-02 deg2rad(0.2)...
    1e-04 1e-04 deg2rad(0.1) 1e-01 1e-01]);

% Process noise covariance.
Q = diag([1e-04 1e-04 deg2rad(0.02) 1e-06 1e-06 deg2rad(0.02)...
    1e-09 1e-09 deg2rad(0.01) 1e-05 1e-05]);

% Measurement noise covariance.
R = diag([2.5^2 2.5^2 deg2rad(0.2) deg2rad(0.2) 1e-01 1e-01]);
...
%% Estimation
   ...
   x1n(3) = max(x1n(3),0.1); % Lower limit on friction
...
%% Detection Algorithm
   ...
   if t(ct) < 10  % Ts = 0.01 seconds
       ...
       fMean(ct) = mean( xSigEst(3, idx) );
       fSTD(ct)  = std( xSigEst(3, idx) );
   else
       ...
       % friction changes.
       estFriction(ct) = mean(xSigEst(3,max(1,ct-10):ct));
...
% 
% figure; % measurement error
% plot(t,ySig(att_state,:)-ySigEst(att_state,:),'b','LineWidth',LineW); 
% grid on; ax = gca; ax.FontSize = FontS_axis;
% xlabel('time (s)','Fontsize',FontS_label); ylabel('angle (rad)','Fontsize',FontS_label);

figure; % Test statistics
subplot(3,2,1); plot(t,t1_r1(1,:),'b',...
    [t nan t],[t1_r1(2,:)+3*t1_r1(3,:),nan,t1_r1(2,:)-3*t1_r1(3,:)],...
    'r--','LineWidth',LineW);
title('North Position')
ylabel('S_{R1}(k)'); axis([0,60,-5,10]);
grid on; ax = gca; ax.FontSize = FontS_axis;
subplot(3,2,3); plot(t,t1_r2(1,:),'b',...
    [t nan t],[t1_r2(2,:)+3*t1_r2(3,:),nan,t1_r2(2,:)-3*t1_r2(3,:)],...
    'r--','LineWidth',LineW);  
ylabel('S_{R2}(k)'); axis([0,60,-5,10]);
grid on; ax = gca; ax.FontSize = FontS_axis;
subplot(3,2,5); plot(t,t1_r3(1,:),'b',...
    [t nan t],[t1_r3(2,:)+3*t1_r3(3,:),nan,t1_r3(2,:)-3*t1_r3(3,:)],...
    'r--','LineWidth',LineW); 
ylabel('S_{R3}(k)'); axis([0,60,-5,10]);
grid on; ax = gca; ax.FontSize = FontS_axis;
xlabel('time (s)');

subplot(3,2,2); plot(t,xSigEst(3,:),'b',...
    [t nan t],[fMean+3*fSTD,nan,fMean-3*fSTD],'r--','LineWidth',LineW);
title('Heading Angle')
axis([0,60,-1,1]);
grid on; ax = gca; ax.FontSize = FontS_axis;
subplot(3,2,4); plot(t,t2_r2(1,:),'b',...
    [t nan t],[t2_r2(2,:)+3*t2_r2(3,:),nan,t2_r2(2,:)-3*t2_r2(3,:)],...
    'r--','LineWidth',LineW);   
axis([0,60,-1,1]);
grid on; ax = gca; ax.FontSize = FontS_axis;
subplot(3,2,6); plot(t,t2_r3(1,:),'b',...
    [t nan t],[t2_r3(2,:)+3*t2_r3(3,:),nan,t2_r3(2,:)-3*t2_r3(3,:)],...
    'r--','LineWidth',LineW); 
axis([0,60,-1,1]);
grid on; ax = gca; ax.FontSize = FontS_axis; xlabel('time (s)'); 
